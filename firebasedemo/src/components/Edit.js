...
class Edit extends Component {
  ...
  componentDidMount() {
    const ref = Firebasedemo.firebasedemo().collection('boards').doc(this.props.match.params.id);
    ref.get().then((doc) => {
      if (doc.exists) {
  ...
  onSubmit = (e) => {
    ...
    const updateRef = Firebasedemo.firebasedemo().collection('boards').doc(this.state.key);
    updateRef.set({
      title,
      description,
      author
